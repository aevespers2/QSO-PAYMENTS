...
- **Next objective:** Approve the documentation-only payment boundary, the portable device-trust/financial-authority separation, and the cross-repository gluing model before any executable payment simulation.
...
- **MVP scope:** approved purpose, users, terminology, jurisdictional assumptions, data/privacy/license model, environment labels, subsystem ownership, financial-approval separation, device/workspace binding, loss/theft/replacement behavior, obstruction ledger, pairwise contracts, triple-overlap witnesses, documentation review, exact-source validation, checksums, provenance, and rollback.
- **Priority:** Charter, authority separation, portable-trust compatibility, and contract evidence precede schemas, accounting engines, adapters, testnet integration, or production claims.
- **Success criteria:** every economic claim is evidence-classified; device enrollment, workspace identity, proposal, authorization, capability, execution, receipt, reconciliation, and canonical disposition remain distinct; wrong-device and revoked-device use fails closed; no credential, custody, signing, transfer, self-authorization, or guaranteed-return capability exists; accepted exact-head evidence and rollback guidance are retained.
...
- **Release rationale:** Financial language, device trust, and automation create elevated security, legal, privacy, and user-expectation risk. A precise, glueable documentation baseline is the safest useful first product.
...
PR #1 now contains a Pages-oriented documentation package, a portfolio-level obstruction and gluing analysis, and a dedicated portable trust/financial authority boundary. The new profile prevents a trusted device or Repository `1` enrollment from being represented as financial approval, defines separate device/workspace and economic subjects, and records wrong-device, lost-device, revocation, replacement, privacy, cache invalidation, and recovery witnesses. This is candidate input to P0–P1A, not completion of those gates.
